...
class General {
    ...
    constructor(data) {
        ...
        if (this.rawData && data["GASAIDS"]) {
            this.attackAbilities = this.rawData.attackSlots ? parseAssignedAbilities(this.rawData.attackSlots.toString().split(',').map(slot => parseInt(slot)), data["GASAIDS"]) : [];
            this.defenseAbilities = this.rawData.defenseSlots ? parseAssignedAbilities(this.rawData.defenseSlots.toString().split(',').map(slot => parseInt(slot)), data["GASAIDS"]) : [];
        }
