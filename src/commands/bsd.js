...
function parseBSD(client, params) {
    ...
    const spyResources = (params.R ?? []).map(r => new Good(r));
 ...
 * @param {BaseClient} client
 * @param {Object} data
 * @returns {{army: { left: InventoryItem<Unit>[], middle: InventoryItem<Unit>[], right: InventoryItem<Unit>[], keep: InventoryItem<Unit>[], supportTools: InventoryItem<Unit>[], stronghold: InventoryItem<Unit>[]}, spyTime: Date, defenderBaron: Lord, defenderGeneral?: General, defenderLegendSkills: []}}
 */
function parseSpyArmyInfo(client, data) {
    const spyLogArmyParamArray = data["S"];
    ...
    const unitsStronghold = parseSingleFlank(4, spyLogArmyParamArray);
    const supportTools = parseSingleFlank(5, spyLogArmyParamArray);
